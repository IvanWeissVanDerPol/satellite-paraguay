...
class TestYvyraSoyPipeline:
    ...
    def test_pueblo_top5_matches_documented(self):
        ...
        assert 400 <= top5_total <= 500, (
            f"Top 5 pueblos total = {top5_total}, expected ~460"
        )
    ...
    def test_no_fabricated_pueblo_names(self):
        ...
        real_pueblos = {
            "Ache", "Ava Guarani", "Mbya Guarani", "Pai Tavytera",
            "Guarani Occidental / Pueblo Guarani", "Guarani Nandeva",
            "Enlhet Norte", "Enxet Sur", "Sanapana", "Angaite",
            "Guana", "Toba Maskoy / Toba Enenlhet", "Nivacle",
            "Maka", "Manjui", "Ayoreo", "Ybytoso", "Tomaraho", "Qom",
        }
        ...
        for p in actual:
            ...
            stripped = p.replace(" ", "").replace("í", "i").replace("é", "e").replace("á", "a").replace("ó", "o").replace("ú", "u").replace("ñ", "n").replace("Ñ", "N")
            for real in real_pueblos:
                real_stripped = real.replace(" ", "").replace("í", "i").replace("é", "e").replace("á", "a").replace("ó", "o").replace("ú", "u").replace("ñ", "n").replace("Ñ", "N")
...
class TestYvyraSoyDataLineage:
    ...
    def test_paper_tex_does_not_claim_circular_unverified_territory_numbers(self):
        ...
        for placeholder in ["49.45", "49.43", "46.46"]:
            assert placeholder not in paper, (
                f"P0030 paper.tex contains P0012 placeholder {placeholder!r}"
            )
