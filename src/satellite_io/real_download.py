...
def download_sentinel2_gee(
    ...
    return {
        ...
        "bands": bands,  # type: ignore
        "source": "GEE",  # type: ignore
        "tile_id": tile_id,  # type: ignore
...
def generate_synthetic_sentinel2(
    ...
    monthly_ndvi = np.array(monthly_ndvi)  # type: ignore
    ...
    return {
        "data": arr,
        "dates": [m.isoformat() for m in months],  # type: ignore
        "bands": bands,  # type: ignore
        "source": "synthetic",  # type: ignore
        "tile_id": tile_id,  # type: ignore
        "bbox": bbox,  # type: ignore
        "seed": seed,  # type: ignore
        "ndvi_baseline": monthly_ndvi.tolist(),  # type: ignore
    }
...
def atmospheric_correction(arr: np.ndarray) -> np.ndarray:
    ...
    if arr.ndim == 4:
        ...
        return np.clip(arr - min_vals, 0, 1)  # type: ignore
    else:
