...
def run_yvytu_demo(data: np.ndarray = None):
    ...
    if data is None:
        raise FileNotFoundError(
            "No NDVI `data` provided to run_yvutu_demo(). "
            "Pass a (T=24, H=224, W=224) NDVI raster from "
            "data/cache/sentinel2/. Or run scripts/run_real_experiment_p0011.py "
            "which downloads from Planetary Computer first. "
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        try:
            ...
            ndvi_in = np.load(sys.argv[1])
        run_yvytu_demo(data=ndvi_in)
    else:
        try:
            run_yvytu_demo()
