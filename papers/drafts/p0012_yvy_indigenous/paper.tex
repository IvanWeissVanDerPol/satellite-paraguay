...
average, with a measured ratio of 3.0$\times$ (24.7\% loss inside vs.
8.5\% outside, $\chi^2 = 460{,}597$, df = 9, $p<0.001$, 95\% CI
[1.72, 4.20]$\times$). The result is sensitive to the definition of
``forest,'' as prior global work~...\citep{sze2022} has documented
similar sensitivity, but the direction is robust across attribution
windows. The core contribution is operational: a defensible attribution
audit that can be reproduced against publicly available Paraguayan
datasets. Two limits are explicit. First, the analysis is
observational; deforestation rate differences are reported as
conditional statistics, not as causal estimates. Second, the FPIC
requirement for any forward deployment of these methods in Indigenous
