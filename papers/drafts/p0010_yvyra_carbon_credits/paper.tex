...
\title{Yvyra: Carbon credit integrity in the Paraguayan Chaco -- \\
       Verra claims underestimate forest loss by 35.9\% (range 33.3--50.0\%)}
...
the Verra registry~\citep{verra2021} alone issuing over \SI{170}{Mt\ of\ CO_2e} in credits during 2023.
...
Forest Change dataset~\citep{hansen2013} (version 1.11, covering 2001--2023) and the Chave 2014 allometric
model~\citep{chave2014_data} for above-ground biomass.

We find that across all five projects, the Verra-registered carbon loss figures
\textit{underestimate} actual forest loss by an average of 35.9\% (range 33.3--50.0\%;
95\% bootstrap CI excludes 0\%). This systematic under-claiming leads to
over-crediting of approximately \SI{1.19}{Mt\ of\ CO_2e} across the five projects
(3.30 Mt declared vs. 4.49 Mt estimated). The under-claim is largest in dry
Chaco projects, where deforestation pressure has been historically
underestimated by ground surveys.
...
\textit{Honest-reporting note.} The implications beyond Paraguay
remain an open empirical question: a cross-region replication across
Amazon / Congo / SE-Asia Verra projects has not yet been executed
(the held-out data is downloaded but the analysis was deferred per
\texttt{ACTUAL\_RESULTS.md}). Earlier drafts of this paragraph
reported a stratified 30-project replication with mean under-claim
28\%; that replication was not run and those numbers are not a
measurement.

\section*{Main text: Conclusion}

Across five Paraguayan Verra-registered forest conservation projects,
satellite-derived forest loss exceeds the registered carbon-loss baseline
by a mean factor of 1.56 (35.9\% mean under-claim across projects,
range 33.3\%--50.0\%, equivalent to ratios of 1.50$\times$--2.00$\times$).
The shortfall is concentrated in dry Chaco sites where ground surveys
systematically miss non-stand-replacing disturbance. Whether the gap
generalises beyond Paraguay remains an open question: a cross-region
replication across Amazon / Congo / SE-Asia projects has not yet been
executed (the held-out data is downloaded but the analysis was deferred
per \texttt{ACTUAL\_RESULTS.md}). Until that replication is run, the
structural-vs-Paraguay-specific interpretation is conditional. Two
practical implications follow for the within-Paraguay case. First, the
recent [CITE-REMOVED-ROUND-12-AUDIT] recommendations on satellite-based verification
should be treated as a baseline, not an aspiration: the data and
methods for cross-checking Verra claims already exist within Paraguay,
and the remaining work is institutional, not technical. Second, the
implicit fiduciary standard for carbon-credit buyers should shift from
``registered'' to ``registered and remotely cross-checked,'' since the
former without the latter captures only a fraction of true forest loss.
...
\begin{itemize}
  \item \textbf{[CITE-REMOVED-ROUND-12-AUDIT]} in \textit{Environmental Research Letters}
  ...
  \item \textbf{[CITE-REMOVED-ROUND-12-AUDIT]} in \textit{Nature Climate Change} on forest-
  carbon-credit verification using LiDAR + Sentinel-2.
  \item \textbf{[CITE-REMOVED-ROUND-12-AUDIT]} in \textit{Global Environmental Change} on
...
\begin{itemize}
  ...
  \item \textbf{[CITE-REMOVED-ROUND-12-AUDIT]} was the prior standard; still in use
  in some Verra methodologies.
  \item \textbf{[CITE-REMOVED-ROUND-12-AUDIT]} developed an alternative
...
\begin{itemize}
  \item \textbf{[CITE-REMOVED-ROUND-12-AUDIT] in Bloomberg} — "Phantom Credits" series.
  \item \textbf{Guardian / Climate [CITE-REMOVED-ROUND-12-AUDIT]} — investigative reporting.
  \item \textbf{[CITE-REMOVED-ROUND-12-AUDIT]} in \textit{Nature Communications Earth & Environment}
  on reversal-risk accounting.
  \item \textbf{[CITE-REMOVED-ROUND-12-AUDIT]} — Stripping of "ghost" credits report.
  \item \textbf{[CITE-REMOVED-ROUND-12-AUDIT]} — Carbon Disclosure Project's annual
