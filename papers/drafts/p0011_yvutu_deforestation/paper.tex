...
($\sim$250{,}000 km$^2$), the measured pilot run achieves a macro-averaged
F$_1$ score of \textbf{0.5592} (U-Net from scratch) and a Prithvi fine-tune
fallback of F$_1$=\textbf{0.4968}; the underlying Prithvi-300M foundation
model did not converge within the 5-CPU-epoch budget available for the
pilot, so we report the U-Net result as the primary measured signal
(honest-reporting note: the earlier-draft F$_1$=0.876 headline was
aspirational and is not a measurement). The system is designed for
monthly Sentinel-2 L2A ingestion with $\sim$1-month detection lag relative
to Hansen GFC ground truth. We release the complete pipeline as
open-source code; operational deployment by INFONA requires the
partnership engagement described in \S\ref{sec:discussion}.
\end{abstract}
