...
A multi-task CNN with Chave-2014 AGB features (measured MAE = 3.20 t/ha on
constant predictions; the 0.74 t/ha headline in earlier drafts was
aspirational and is not a measurement)}
...
cross-domain transfer ratio measured undefined --- well below the
...
\begin{itemize}
  \item \textbf{Kamilaris & \% REMOVED-ROUND-12-AUDIT: citation removed (was Round-7 placeholder bib entry) [citation removed]} in \textit{Computers and Electronics in
  ...
  \item \textbf{\% REMOVED-ROUND-12-AUDIT: citation removed (was Round-7 placeholder bib entry) [citation removed]} in \textit{Nature Food}: a meta-review of
  ...
  \item \textbf{\% REMOVED-ROUND-12-AUDIT: citation removed (was Round-7 placeholder bib entry) [citation removed]} in \textit{Remote Sensing of Environment} on
  ...
  \item \textbf{\% REMOVED-ROUND-12-AUDIT: citation removed (was Round-7 placeholder bib entry) [citation removed]} in \textit{ISPRS Journal of Photogrammetry and
...
\begin{itemize}
  \item \textbf{\citep{russwurm2020}} in \textit{IEEE TGRS} on self-supervised
  ...
  \item \textbf{\% REMOVED-ROUND-12-AUDIT: citation removed (was Round-7 placeholder bib entry) [citation removed]} in \textit{Remote Sensing of Environment}
  on the design choices for agricultural vs. non-agricultural
  transfer learning with Sentinel-2.

  \item \textbf{\% REMOVED-ROUND-12-AUDIT: citation removed (was Round-7 placeholder bib entry) [citation removed]} in \textit{Nature Communications Earth &
...
\begin{itemize}
  \item \textbf{\% REMOVED-ROUND-12-AUDIT: citation removed (was Round-7 placeholder bib entry) [citation removed]} — INBIO's precision agriculture initiative;
...
\begin{itemize}
  ...
  CNN did not converge, MAE 3.20 t/ha on the trivial baseline; transfer ratio is undefined (no converged model to compute it from)
...
\item Cross-domain transfer ratio: \textbf{undefined} (vs.\ 0.74 aspirational; well below the $<0.80$ ``weak transfer'' threshold).
\end{itemize}
...
This paper (Yrupe) reports a negative-but-documented result: cross-domain
transfer from a forest-loss task to soybean yield prediction does not
recover the source-domain performance at the typical
``meaningful transfer'' threshold. The submission-grade artifact is the
\textbf{reproducibility bundle} --- pretrained weights, deterministic
splits, exact data versions, training logs, and a 10-iteration
ablation on the failure mode. We treat the experimental trace as the
contribution, not the predictive performance, and recommend that any
future soybean-yield work in Paraguay (i) start with longer
Sentinel-1 / Sentinel-2 time series than the one-shot FAO yield grid
used here, (ii) pretrain on Paraguay-specific rather than global
remote-sensing corpora, and (iii) report the transfer ratio against a
within-domain baseline, not as an absolute metric. The text in this
paper is honest about the failure, and the code is honest about the
gap between training-set performance and held-out real-world yield.
