        ...
        for line in f:
            ...
            if line and not line.startswith('#'):
                ...
                if m:
                    ...
                    entries.append({'surname': surname, 'author': author, 'year': year, 'title': title})
    return entries

def sign_score(a, b):
    """Score two title strings - return best of: direct overlap, sorted-words overlap, stem overlap, year match bonus."""
    # Strip punctuation, lowercase
    clean = lambda s: re.sub(r'[^a-z0-9\u00C0-\u00FC\s]', ' ', s.lower())
...
def crossref_query(surname, year, title):
    ...
    try:
        ...
        if result.returncode != 0:
            return {'matches': [], 'error': f'curl exit {result.returncode}', 'best_score': 0}
        ...
        for item in items:
            ...
            matches.append({
                ...
                'year': year_match,
                'authors': [f"{a.get('family', '')}, {a.get('given', '')}" for a in (item.get('author', []) or [])][:3],
                'score': round(score, 3)
            })
...
with open(OUT_DIR / 'verification_report_v3.md', 'w') as f:
    ...
    f.write("**Method:** CrossRef API via curl, ±1 year filter, two-tier scoring (Jaccard ×2 + min-overlap), +0.2 year bonus\n")
    f.write("**Source:** `citations_to_verify.txt` (226 candidates extracted from round-2 files)\n\n")
    f.write("## Statistics\n\n")
    total = len(entries)
    f.write(f"- Total candidates: {total}\n")
    f.write(f"- ✅ **VERIFIED** (>0.6): {len(verified)} ({100*len(verified)/total:.0f}%)\n")
    f.write(f"- ⚠️ **LIKELY** (0.4-0.6): {len(likely)} ({100*len(likely)/total:.0f}%)\n")
    f.write(f"- 📝 **PARTIAL** (0.2-0.4): {len(partial)} ({100*len(partial)/total:.0f}%)\n")
    f.write(f"- ❌ **NOT_FOUND** (<0.2): {len(not_found)} ({100*len(not_found)/total:.0f}%)\n\n")

    if verified:
        f.write("## ✅ VERIFIED — Add to thesis bibliography\n\n")
        for r in verified:
            m = r['matches'][0]
            f.write(f"- **{r['author']}** → DOI: `{m.get('doi', '?')}` (score {r['best_score']})\n")
    ...
    if likely:
        ...
        for r in likely[:50]:
            ...
            f.write(f"- **{r['author']}** (score {r['best_score']}) → possible DOI: `{m.get('doi', '?')}`\n")

    if partial:
        f.write(f"\n## 📝 PARTIAL — Title doesn't match well, may be a real paper with different title\n\n")
        for r in partial[:50]:
            m = r['matches'][0] if r['matches'] else {}
            f.write(f"- **{r['author']}** (score {r['best_score']}) → unrelated top hit: {m.get('title', '')[:100]}\n")
